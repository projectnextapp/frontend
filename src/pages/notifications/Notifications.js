...
import { PageHeader, Button, Card, Badge, PageLoader, EmptyState } from '../../components/ui/UI';
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          unread > 0 && (
            <Button variant="ghost" icon={<MdDoneAll />} onClick={markAllRead}>Mark All Read</Button>
          )
        }
      />
      ...
      {notifications.length === 0 ? (
        <Card>
          <EmptyState icon={<MdNotifications />} title="No notifications" description="You're all caught up!" />
        </Card>
      ) : (
        <Card padding={false}>
